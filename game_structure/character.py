...
class Tom(pygame.sprite.Sprite):
    ...
    def __init__(
        ...
        if footprint_img_directory:
            for file in os.listdir(os.path.join(footprint_img_directory, "Solution")):
                ...
                self.footprint_images.append(image)
        self.foot = self.footprint_images[0]
        ...
        for folder in os.listdir(
            ...
            for file in os.listdir(os.path.join(tom_img_directory, folder)):
                tmp_img = pygame.image.load(
                    os.path.join(tom_img_directory, folder, file)
                ).convert_alpha()
    ...
    def is_valid_move(self, direction: str, grids) -> bool:
        if (
            get_position_after_move(position=self.position, direction=direction)
            in grids[self.position].get_neighbors()
        ):
            return True
        return False
    ...
    def update(
        ...
        **kwargs
        ...
        if direction == "T":
            self.direction = "T"
            self.normal_move(
                self.animation_images["Up"],
                direction=direction,
                maze=maze,
                energy_grp=energy_grp,
                jerrgy_grp=jerry_grp,
                ui_grp=ui_grp,
            )
        elif direction == "B":
            self.direction = "B"
            self.normal_move(
                self.animation_images["Down"],
                direction=direction,
                maze=maze,
                energy_grp=energy_grp,
                jerrgy_grp=jerry_grp,
            ...
            if self.direction == "T":
                ...
                if int(self.current_sprite) >= len(self.animation_images["StandDown"]):
                    self.current_sprite = 0
                self.image = self.animation_images["StandDown"][
                ...
                if int(self.current_sprite) >= len(self.animation_images["StandLeft"]):
                    self.current_sprite = 0
                self.image = self.animation_images["StandLeft"][
                    int(self.current_sprite)
                ]
                ...
                if int(self.current_sprite) >= len(self.animation_images["StandRight"]):
                    self.current_sprite = 0
                self.image = self.animation_images["StandRight"][
                    int(self.current_sprite)
                ]
            else:
                if no_event:
...
class Jerry(Tom):
    def __init__(
        ...
        for folder in os.listdir(jerry_img_directory):
            for file in os.listdir(os.path.join(jerry_img_directory, folder)):
                tmp_img = pygame.image.load(
                    os.path.join(jerry_img_directory, folder, file)
                )
    ...
    def escape_move(self, maze, energy_grp=None, tom_grp=None, ui_grp=None):
        ...
        for direction in directions:
            if self.is_valid_move(direction=direction, grids=maze.grids):
                next_position = get_position_after_move(
                    position=self.position, direction=direction
                )
        ...
        if distances:
            maximize_direction = sorted(
                list(distances.keys()), key=lambda x: distances[x], reverse=True
            )[0]
            ...
            self.position = get_position_after_move(
                position=self.position, direction=maximize_direction
            )

            move_coord = get_diffirent_coord(
                direction=maximize_direction, maze_grid_size=self.grid_size
            )
            move_coord = move_coord / self.grid_size
        ...
        for i in range(maze.maze_size):
            for j in range(maze.maze_size):
                ...
                if mahathan_distance(
                    (i, j), tom_grp.sprite.position
                ) > current_distance and BDFS(
                    grids=maze.grids,
