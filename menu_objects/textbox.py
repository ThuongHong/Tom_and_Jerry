...
class TextBox:
    def __init__(self, x_coord, y_coord, length, width, image_source, sound_source):
        ...
        self.eye1_button = button.Button(
            self.x_coord + 270, self.y_coord + 23, self.eye1_img, self.sound, 0.1, 0.11
        )
        self.eye2_button = button.Button(
            self.x_coord + 270, self.y_coord + 23, self.eye2_img, self.sound, 0.1, 0.11
        )
    ...
    def draw_text(self, surface, cursor_color, is_password, censored, activated=False):
        ...
        if activated:
            ...
            if time.time() % 1 > 0.5:
                surface.blit(
                    cursor,
                    (self.x_coord + text.get_rect().width + 10, self.y_coord + 10),
                )
    ...
    def get_text(
        ...
        while activated:
                ...
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_RETURN or event.key == pygame.K_ESCAPE:
                            ...
                            self.text
                        ) < DISPLAY.TEXT_LENGTH and TextBox.is_valid_char(
                            self, event.unicode
                        ):
            ...
            TextBox.draw_text(
                self, surface, COLOR.BLACK, is_password, censored, activated
            )
