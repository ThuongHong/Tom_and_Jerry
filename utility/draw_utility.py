...
def draw_two_grids(grids, screen, current_grid: tuple[int], next_grid: tuple[int]):
...
def mark_grid(
    ...
    if footprint:  # Solution
        ...
        tmp_img = pygame.image.load(os.path.join("images", "Tom", "StandDown", "1.png"))

        tmp_img_height = tmp_img.get_height()
        tmp_img_width = tmp_img.get_width()

        bigger_size = (
            tmp_img_height if (tmp_img_height > tmp_img_width) else tmp_img_width
        )
